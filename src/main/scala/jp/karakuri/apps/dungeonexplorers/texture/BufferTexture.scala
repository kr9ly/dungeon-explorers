...
class BufferTexture(val width:Int, val height:Int)(implicit val gl:GL) extends Texture {
  ...
  gl.glTexImage2D(GLConsts.GL_TEXTURE_2D, 0, GLConsts.GL_RGBA, width, height, 0, GLConsts.GL_RGBA, null.asInstanceOf[ByteBuffer])
